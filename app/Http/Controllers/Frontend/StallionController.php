...
class StallionController extends Controller
{
    public function stallion(Request $request)
    {
        ...
        }])->where('category','stallions')->where('status',1)->where('created_at','>=',$date)->where('payment_status','paid')->orderBy('id','DESC')->get();
        ...
        if ($request->ajax()) {
            ...
            $stallions = Stallion::with(['stallionImages' => function ($query) {
                $query->orderBy('new_element', 'DESC');
            }])->where('category','stallions')->where('status',1)->where('created_at','>=',$date)->where('payment_status','paid')->orderBy('id', 'ASC')->paginate(2);
            return view('frontend.stallionlist', compact('categories', 'stallions','featureStatus','stalliondetails'));
        }

        $stallions = Stallion::with(['stallionImages' => function ($query) {
            $query->orderBy('new_element', 'DESC');
        }])->where('category','stallions')->where('created_at','>=',$date)->where('payment_status','paid')->where('status',1)->orderBy('id', 'ASC')->paginate(2);

 
        $advertisements=advertisement::select('image','link')->where('page','Stallion')->get();
        return view('frontend.stallion', compact('categories', 'stallions','featureStatus','stalliondetails','advertisements'));
